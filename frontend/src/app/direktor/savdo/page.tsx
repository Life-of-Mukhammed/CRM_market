...
export default function DirektorSavdo() {
  const [page, setPage] = useState(1);
  const [selectedSale, setSelectedSale] = useState<Sale | null>(null);
  const [receiptSale, setReceiptSale] = useState<Sale | null>(null);

  const { data } = useQuery({
    queryKey: ['all-sales', page],
    queryFn: () => salesApi.list({ page, limit: 20 }).then((r) => r.data),
  });

  const { data: todayData } = useQuery({
    queryKey: ['all-sales-today'],
  ...
  const kassirName = (sale: Sale) => typeof sale.kassir === 'string' ? '—' : sale.kassir.name;
  ...
  return (
    <DashboardLayout>
      <div className="p-4 lg:p-6 space-y-6">
        {/* Header */}
        <div>
          <h1 className="text-2xl font-black text-gray-900 dark:text-white">Барча савдолар</h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Барча кассирлар томонидан қилинган сотувлар</p>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100 dark:border-dark-700">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Кассир</th>
        ...
        <Modal open={!!selectedSale} onClose={() => setSelectedSale(null)} title={`Сотув: ${selectedSale.saleNumber}`}>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <p className="text-gray-500">Кассир</p>
                <p className="font-semibold">{kassirName(selectedSale)}</p>
              </div>
              <div>
